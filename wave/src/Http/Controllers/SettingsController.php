...
class SettingsController extends Controller
{
    ...
    public function index($section = '')
    {
        ...
        // $invoices1 = PaddleSubscription::where('user_id', auth()->user()->id)->get();
        $invoices1 = PaddleSubscription::leftJoin('plans', 'paddle_subscriptions.plan_id', '=', 'plans.id')
    ->where('paddle_subscriptions.user_id', auth()->user()->id)
    ->select('paddle_subscriptions.*', 'plans.price as plan_price')
    ->get();

        if (empty($section)) {
            return redirect(route('wave.settings', 'profile'));
        }
        return view('theme::settings.index', compact('section', 'invoices1'));
    }
    public function admininvoices()
    {
        ...
        if ($admin) {
            $tickets = PaddleSubscription::leftJoin('plans', 'paddle_subscriptions.plan_id', '=', 'plans.id')
                ->leftJoin('users', 'paddle_subscriptions.user_id', '=', 'users.id')
                ->select('paddle_subscriptions.*', 'plans.price as plan_price', 'plans.name as plan_name', 'users.email as user_email')
                ->get();
        } else {
            $tickets = PaddleSubscription::leftJoin('plans', 'paddle_subscriptions.plan_id', '=', 'plans.id')
                ->leftJoin('users', 'paddle_subscriptions.user_id', '=', 'users.id')
        ...
        return view('theme::settings.partials.admin-invoices', compact('tickets'));
    }
    ...
    public function profilePut(Request $request)
    {
        ...
        $authed_user->fname = $request->fname;
        $authed_user->lname = $request->lname;
        $authed_user->company = $request->company;
        $authed_user->phone = $request->phone;
        $authed_user->country = $request->country_id;
        $authed_user->state = $request->state_id;
        $authed_user->city = $request->city_id;
        $authed_user->zip = $request->zip;
        $authed_user->address = $request->address;
        if ($request->avatar) {
    ...
    // {
    //     $invoice = PaddleSubscription::find($invoiceId);
